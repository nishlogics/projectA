...
class StageCalc:
    ...
    mass_ratio: Optional[float] = None      # report theoretical: out_g/in_g

    def compute(self) -> None:
        if self.plant_batch_kg is None:
            self.plant_batch_kg = self.lab_input_g * self.scale_up_ratio / 1000.0
        # moles in kgmol (batch kg / MW) — consistent for the kg-based mass balance
        ...
        if self.mass_ratio is not None:
            # faithful to the report: output = input batch x (theoretical out/in)
            self.output_kg = self.plant_batch_kg * self.mass_ratio
            self.output_moles = self.output_kg / self.product_mw
        else:
    ...
    @property
    def moles_report(self) -> Optional[float]:
        """Moles in the convention the report prints them (grams / MW), i.e. mol.
        This is 1000x the kgmol figure and matches the report's 'Mol.' column."""
        if self.input_moles is None:
            return None
        return self.input_moles * 1000.0
